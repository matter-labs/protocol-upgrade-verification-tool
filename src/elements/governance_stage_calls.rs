...
impl GovernanceStage1Calls {
    ...
    pub async fn verify(
        ...
        gw_old_chain_creation_params: Option<&OldChainCreationParams>,
        ...
        // and, only when the upgrade has a Gateway leg (gateway_chain_id != 0):
        ...
        let has_gateway = gateway_chain_id != 0;
        ...
        if has_gateway {
            let list_of_calls: Vec<_> = l1_only_calls
                .iter()
                .chain(gateway_calls.iter())
                .copied()
                .collect();
            self.calls.verify(&list_of_calls, verifiers, result)?;
        } else {
            self.calls.verify(&l1_only_calls, verifiers, result)?;
        }
        ...
        // Without a Gateway leg there is nothing more to verify in stage 1.
        if !has_gateway {
            return Ok((
        ...
        let gw_old_chain_creation_params = gw_old_chain_creation_params
            .expect("old_chain_creation_params.gateway is required when gateway_chain_id != 0");
        ...
        Ok((
            l1_chain_creation_diamond_cut,
            l1_force_deployments,
            Some(gw_chain_creation_diamond_cut),
            Some(gw_force_deployments),
        ))
    }
}
...
impl GovernanceStage0Calls {
    ...
    pub(crate) async fn verify(
        ...
        // Stage 0 handles pausing migration on L1 (and on Gateway, when the
        // upgrade has a Gateway leg) and starting the upgrade timer.
        let has_gateway = gateway_chain_id != 0;
        ...
        if has_gateway {
            // Verify approve base token
            {
                ...
                let data = approveCall::abi_decode(&calldata, true)
...
impl GovernanceStage2Calls {
    ...
    pub(crate) async fn verify(
        ...
        // Stage2 is where we check the upgrade presence and unpause migration
        // on l1 (and on gateway, when the upgrade has a Gateway leg)
        let has_gateway = gateway_chain_id != 0;
